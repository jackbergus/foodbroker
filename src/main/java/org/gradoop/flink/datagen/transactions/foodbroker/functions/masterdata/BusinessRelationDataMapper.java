...
public class BusinessRelationDataMapper
  ...
  public Tuple2<GradoopId, BusinessRelationData> map(Vertex v) throws Exception {
    reuseBusinessRelationData
      .setQuality((Float)v.getPropertyValue(FoodBrokerPropertyKeys.QUALITY_KEY));
    reuseBusinessRelationData
      .setCity(v.getPropertyValue(FoodBrokerPropertyKeys.CITY_KEY).toString());
    reuseBusinessRelationData
      .setHolding(v.getPropertyValue(FoodBrokerPropertyKeys.HOLDING_KEY).toString());

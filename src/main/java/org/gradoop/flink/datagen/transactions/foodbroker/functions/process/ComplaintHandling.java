 ...
 */
public class ComplaintHandling extends AbstractProcess implements Function<GraphTransaction, GraphTransaction> {
   ...
   */
  public ComplaintHandling(FoodBrokerConfig config) {
    super(config);
  }
  ...
  @Override
  public GraphTransaction apply(GraphTransaction graph) {
  ...
  private void badQuality(Set<Vertex> deliveryNotes, GraphTransaction graph) {
    ...
    for (Vertex deliveryNote : deliveryNotes) {
      influencingMasterQuality = new ArrayList<>();
      badSalesOrderLines = new HashSet<>();
      ...
      for (Edge purchOrderLine : currentPurchOrderLines) {
        influencingMasterQuality.add(getQuality(productIndex, purchOrderLine.dst));
      }
  ...
  private void lateDelivery(Set<Vertex> deliveryNotes, GraphTransaction graph) {
    Set<Edge> lateSalesOrderLines = new HashSet<>();

    // Iterate over all delivery notes and take the sales order lines of
    // sales orders, which are late
    for (Vertex deliveryNote : deliveryNotes) {
      if (((LocalDate)deliveryNote.getPropertyValue(FoodBrokerPropertyKeys.DATE_KEY)).isAfter(((LocalDate)salesOrder.getPropertyValue(FoodBrokerPropertyKeys.DELIVERYDATE_KEY)))) {
    ...
    if (!lateSalesOrderLines.isEmpty()) {
      ...
      Set<Edge> latePurchOrderLines = new HashSet<>();
      ...
      LocalDate d = (LocalDate) salesOrder.getPropertyValue(FoodBrokerPropertyKeys.DELIVERYDATE_KEY);
      calendar.setTimeInMillis(serializeDateTime(d));
  ...
  public static long serializeDateTime(LocalDate dateTime) {
    int year = dateTime.getYear();
    int month = dateTime.getMonth().getValue();
    int day = dateTime.getDayOfMonth();
    Calendar gi = Calendar.getInstance();
    gi.set(year, month, day);
    return gi.getTimeInMillis();
  }
  ...
  private void grantSalesRefund(Set<Edge> salesOrderLines, Vertex ticket) {
    ...
    for (Edge salesOrderLine : salesOrderLines) {
      salesAmount = BigDecimal.valueOf(
              (Integer)salesOrderLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY))
        .multiply(
                (BigDecimal)salesOrderLine.getPropertyValue(FoodBrokerPropertyKeys.SALESPRICE_KEY))
    ...
    if (refundAmount.floatValue() < 0) {
      ...
      properties.set(
        ...
        ticket.getPropertyValue(FoodBrokerPropertyKeys.CREATEDATE_KEY));
  ...
  private void claimPurchRefund(Set<Edge> purchOrderLines, Vertex ticket) {
    GradoopId purchOrderId = purchOrderLines.iterator().next().src;
    ...
    for (Edge purchOrderLine : purchOrderLines) {
      purchAmount = BigDecimal.valueOf((Integer)
        purchOrderLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY))
        .multiply((BigDecimal)purchOrderLine.getPropertyValue(FoodBrokerPropertyKeys.PURCHPRICE_KEY))
        .setScale(2, BigDecimal.ROUND_HALF_UP);
    ...
    if (refundAmount.floatValue() < 0) {
      ...
      properties.set(
        FoodBrokerPropertyKeys.DATE_KEY,
              (Long)ticket.getPropertyValue(FoodBrokerPropertyKeys.CREATEDATE_KEY));
  ...
  private Set<Edge> getPurchOrderLinesByPurchOrder(GradoopId purchOrderId) {
    Set<Edge> currentPurchOrderLines = new HashSet<>();
    for (Edge purchOrderLine : purchOrderLines) {
      if (purchOrderId.equals(purchOrderLine.src)) {
  ...
  private Edge getCorrespondingPurchOrderLine(GradoopId salesOrderLineId) {
    for (Edge purchOrderLine : purchOrderLines) {
      if (purchOrderLine.getPropertyValue("salesOrderLine").toString()
  ...
  private Edge getCorrespondingSalesOrderLine(GradoopId purchOrderLineId) {
    for (Edge salesOrderLine : salesOrderLines) {
      if (salesOrderLine.getPropertyValue("purchOrderLine").toString()
  ...
  private Vertex getUserFromEmployee(Vertex employee, GraphTransaction graph) {
    if (masterDataMap.containsKey(employee.getId())) {
      ...
      String sourceIdKey = properties.get(FoodBrokerPropertyKeys.SOURCEID_KEY).toString();
      ...
      String email = properties.get(FoodBrokerPropertyKeys.NAME_KEY).toString();
      ...
      userMap
        .put(user.getId(), (Float) user.getPropertyValue(FoodBrokerPropertyKeys.QUALITY_KEY));
...
//      newEdge(FoodBrokerConstants.SAMEAS_EDGE_LABEL, client.getId(), customerId);
  ...
  private Map<Tuple2<String, GradoopId>, Set<Edge>> createEdgeMap(GraphTransaction transaction) {
    Map<Tuple2<String, GradoopId>, Set<Edge>> edgeMap = new HashMap<>();
    Set<Edge> edges;
    Tuple2<String, GradoopId> key;
    for (Edge edge : transaction.getEdges()) {
      edges = new HashSet<>();
      key = new Tuple2<>(edge.getLabel(), edge.src);

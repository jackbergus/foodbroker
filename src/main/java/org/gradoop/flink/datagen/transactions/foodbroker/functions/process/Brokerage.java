 ...
 */
public class Brokerage extends AbstractProcess implements Function<Long, GraphTransaction> {
   ...
   */
  public Brokerage(FoodBrokerConfig config) {
    super(config);
  }

  @Override
  public GraphTransaction apply(Long seed) {
    ...
    edgeMap = new HashMap<>();
    GraphTransaction graphHead = graphHeadFactory.createGraphHead();
    ...
    //graphHead.setGraphHead(graphHead);
    graphHead.setVertices(getVertices());
    graphHead.setEdges(getEdges());
    return graphHead;
  }
  ...
  private boolean confirmed(Vertex salesQuotation) {
    List<Float> influencingMasterQuality = new ArrayList<>();
    GradoopId employee =
  ...
  private List<Edge> newSalesQuotationLines(Vertex salesQuotation) {
    List<Edge> salesQuotationLines = new ArrayList<>();
  ...
  private Vertex newSalesOrder(Vertex salesQuotation) {
    ...
    LocalDate salesQuotationDate = (LocalDate) salesQuotation.getPropertyValue(FoodBrokerPropertyKeys.DATE_KEY);
  ...
  private List<Edge> newSalesOrderLines(Vertex salesOrder,
    ...
    List<Edge> salesOrderLines = new ArrayList<>();
    Edge salesOrderLine;
  ...
  private Edge newSalesOrderLine(Vertex salesOrder, Edge salesQuotationLine) {
    ...
    properties.set(FoodBrokerPropertyKeys.SALESPRICE_KEY, salesQuotationLine.getPropertyValue(FoodBrokerPropertyKeys.SALESPRICE_KEY));
    properties.set(FoodBrokerPropertyKeys.QUANTITY_KEY, salesQuotationLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY));

    return newEdge(label, salesOrder.getId(),
      salesQuotationLine.dst, properties);
  }
  ...
  private List<Vertex> newPurchOrders(Vertex salesOrder, List salesOrderLines) {
    List<Vertex> purchOrders = new ArrayList<>();
    Vertex purchOrder;
  ...
  private Vertex newPurchOrder(Vertex salesOrder, GradoopId processedBy) {
    ...
    LocalDate salesOrderDate = (LocalDate) salesOrder.getPropertyValue(FoodBrokerPropertyKeys.DATE_KEY);
  ...
  private List<Edge> newPurchOrderLines(List<Vertex> purchOrders,
    ...
    List<Edge> purchOrderLines = new ArrayList<>();
    Edge purchOrderLine;
    Vertex purchOrder;
  ...
  private Edge newPurchOrderLine(Vertex purchOrder, Edge salesOrderLine) {
    ...
    // calculate and set the properties
    BigDecimal price = getPrice(salesOrderLine.dst);
    ...
    properties.set(FoodBrokerPropertyKeys.QUANTITY_KEY, salesOrderLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY));
    properties.set(FoodBrokerPropertyKeys.PURCHPRICE_KEY, purchPrice);

    purchOrderLine = newEdge(label, purchOrder.getId(), salesOrderLine.dst,
      properties);
  ...
  private List<Vertex> newDeliveryNotes(List<Vertex> purchOrders) {
    List<Vertex> deliveryNotes = new ArrayList<>();
    Vertex deliveryNote;
  ...
  private Vertex newDeliveryNote(Vertex purchOrder) {
    ...
    // calculate and set the properties
    LocalDate purchOrderDate = (LocalDate) purchOrder.getPropertyValue(FoodBrokerPropertyKeys.DATE_KEY);
  ...
  private List<Vertex> newPurchInvoices(List<Edge> purchOrderLines) {
    Vertex purchOrder;
    Map<Vertex, BigDecimal> purchOrderTotals = new HashMap<>();

    BigDecimal total;
    BigDecimal purchAmount;
    // calculate to total cost for each purch order
    for (Edge purchOrderLine : purchOrderLines) {
      purchOrder = vertexMap.get(purchOrderLine.src);
      total = BigDecimal.ZERO;

      if (purchOrderTotals.containsKey(purchOrder)) {
        total = purchOrderTotals.get(purchOrder);
      }
      purchAmount = BigDecimal.valueOf((Integer)purchOrderLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY));
      purchAmount = purchAmount.multiply((BigDecimal) purchOrderLine.getPropertyValue(FoodBrokerPropertyKeys.PURCHPRICE_KEY));
      total = total.add(purchAmount);
      purchOrderTotals.put(purchOrder, total);
    }

    List<Vertex> purchInvoices = new ArrayList<>();
  ...
  private Vertex newSalesInvoice(List<Edge> salesOrderLines) {
    ...
    Vertex salesOrder = vertexMap.get(salesOrderLines.get(0).src);
    Properties properties = new Properties();
    ...
    for (Edge salesOrderLine : salesOrderLines) {
      salesAmount = BigDecimal.valueOf((Integer)salesOrderLine.getPropertyValue(FoodBrokerPropertyKeys.QUANTITY_KEY))
        .multiply((BigDecimal) salesOrderLine.getPropertyValue(FoodBrokerPropertyKeys.SALESPRICE_KEY))
        .setScale(2, BigDecimal.ROUND_HALF_UP);
      revenue = (BigDecimal) salesInvoice.getPropertyValue(FoodBrokerPropertyKeys.REVENUE_KEY);

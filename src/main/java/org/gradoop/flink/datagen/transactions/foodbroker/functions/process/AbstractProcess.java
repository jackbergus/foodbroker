...
import it.giacomobergami.graph.factories.EdgeFactory;
import it.giacomobergami.graph.factories.GraphFactory;
...
public abstract class AbstractProcess extends AbstractRichFunction {
  ...
  ExecutionEnvironment ee = ExecutionEnvironment.instance();

  /**
   * EPGM graph head factory.
   */
  protected GraphFactory graphHeadFactory = GraphFactory.instance();
  /**
   * EPGM vertex factory.
   */
  protected VertexFactory vertexFactory = VertexFactory.instance();
  /**
   * EPGM edge factory.
   */
  protected EdgeFactory edgeFactory = EdgeFactory.instance();
  /**
   ...
   */
  public AbstractProcess(FoodBrokerConfig config) {
    this.config = config;
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    productList = productIndex.keySet().toArray(new GradoopId[productIndex.keySet().size()]);
  }
  ...
  private Map<GradoopId, Vertex> createIndexFromBroadcast(String broadcastVariable) {
    List<Vertex> broadcast = ee.getBroadcastVariable3(broadcastVariable);

    Map<GradoopId, Vertex> map = new HashMap<>(broadcast.size());
  ...
  protected GradoopId getEdgeTargetId(String edgeLabel, GradoopId source) {
    ...
    return edgeMap.get(new Tuple2<>(edgeLabel, source)).iterator().next().dst;
  }
  ...
  protected float getQuality(Map<GradoopId, Vertex> index, GradoopId id) {
    return (Float)index.get(id).getPropertyValue(FoodBrokerPropertyKeys.QUALITY_KEY);
  }
  ...
  protected Float getAdditionalInfluence(
    ...
    case FoodBrokerBroadcastNames.BC_EMPLOYEES:
      firstCity = employeeIndex
        .get(firstMasterDataId).getPropertyValue(FoodBrokerPropertyKeys.CITY_KEY).toString();
      break;
    default:
      break;
    }
  ...
  protected String getStringValue(Map<GradoopId, Vertex> index, GradoopId id, String key) {
    return index.get(id).getPropertyValue(key).toString();
  }
  ...
  protected BigDecimal getPrice(GradoopId productId) {
    return (BigDecimal) productIndex
      .get(productId).getPropertyValue(FoodBrokerPropertyKeys.PRICE_KEY);
  }
}

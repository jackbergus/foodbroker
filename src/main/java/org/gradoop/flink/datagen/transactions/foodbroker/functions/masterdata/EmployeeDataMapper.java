...
public class EmployeeDataMapper implements MapFunction<Vertex, Tuple2<GradoopId, EmployeeData>> {
  ...
  public Tuple2<GradoopId, EmployeeData> map(Vertex v) throws Exception {
    reuseEmployeeData.setQuality((Float)v.getPropertyValue(FoodBrokerPropertyKeys.QUALITY_KEY));
    reuseEmployeeData.setCity(v.getPropertyValue(FoodBrokerPropertyKeys.CITY_KEY).toString());
